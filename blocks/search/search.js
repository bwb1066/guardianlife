...
function renderResult(result, searchTerms, titleTag) {
  ...
  if (result.image) {
    ...
    wrapper.append(createPicture({ src: result.image, breakpoints: [{ width: '375' }] }));
    a.append(wrapper);
  }
  if (result.title) {
...
function compareFound(hit1, hit2) { return hit1.minIdx - hit2.minIdx; }
...
function searchBox(block, config) {
  ...
  box.append(icon, searchInput(block, config));
  import('../../scripts/utils/icons.js').then((mod) => mod.default(box.querySelectorAll('span.icon')));
  return box;
}
